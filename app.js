...
function buildReferenteHTML(brand, color) {
  ...
  const brgb = hexToRgb(bc);
  const label = r.name ? `REF. ${r.name.toUpperCase()}` : "REFERENTE";
  return `
    <div class="brand-ref-separator">
      <span class="brand-ref-sep-line"></span>
      <span class="brand-ref-sep-label">${label}</span>
      <span class="brand-ref-sep-line"></span>
    </div>
    <div class="brand-ref-line" style="width:100%;height:4px;border-radius:2px;position:relative;overflow:hidden;background:linear-gradient(90deg,transparent,rgba(${brgb},0.3) 15%,${bc} 35%,${bcl} 50%,${bc} 65%,rgba(${brgb},0.3) 85%,transparent)"><span class="brand-ref-shimmer"></span></div>
